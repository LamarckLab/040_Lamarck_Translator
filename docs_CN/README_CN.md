...
一个常驻托盘的 **Windows 划词与截图翻译工具**，读任何英文材料都用得上 —— 一篇论文、一份库的文档、一串 issue 讨论、一个报错弹窗。在浏览器或 Zotero 里选中一句话按 `Alt+C`，或者在扫描版 PDF、图表、以及任何不让你选中文字的地方框一个框按 `Alt+S`，结果都以逐句英中对照的形式返回。

它和常见翻译工具的区别在后端：它调用**本机的 `codex` CLI**，直接复用那里已经保存的登录状态。项目本身不配置、不保存、也不读取任何翻译 API Key —— 翻译跑在这台机器上当前登录的那个 Codex 账号上。

默认提示词是按技术阅读调过的，覆盖蛋白质科学、结构生物学、机器学习和软件工程：保留专业术语、缩写、公式、残基编号、链名、文件路径、命令和参数，必要时在中文术语后用括号保留英文。
...
| `prompt`            | 技术翻译风格      | 只管翻译风格；结构化输出格式由程序自动追加                  |
